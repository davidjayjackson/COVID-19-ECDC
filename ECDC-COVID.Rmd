...
ECDC <- ECDC %>% select(countriesAndTerritories,dateRep,year,month,cases,deaths)

colnames(ECDC) <- c("Countries","Reported","Year","Month","Cases","Deaths")  
```
...
df.us %>% plot_ly() %>% add_lines(x=~Reported,y=~TotalCases) %>% layout(title="US Total Confrimed Cases by Day")

df.us %>% plot_ly() %>% add_lines(x=~Reported,y=~TotalDeaths) %>% layout(title="US Total Confrimed Deaths by Day")
